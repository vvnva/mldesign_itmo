...
## 4. Внедрение и инференс решения

Решение реализовано в виде **изолированного ML-сервиса**, пригодного как для офлайн-пилота, так и для
дальнейшей интеграции в инфраструктуру заказчика

### 4.1 Архитектура решения

#### Общая схема

Решение реализовано в виде **одного stateless ML-сервиса**, предоставляющего HTTP API для инференса
...
- Эндпоинт `/model`
- Stateless, без хранения состояния между запросами

**Pattern Classification**
- Классификация входного текста по типу шаблона обращения
- Используется для выбора алгоритма извлечения целевого текста

**Алгоритмический парсинг**
- Rule-based извлечение текста и структурированных полей
- Используется для:
  - извлечения сущностей, если они явно присутствуют в форме
  - fallback при отсутствии NER-сущностей
...
- Извлекает:
  ...
  - ТРК
  - тип топлива и др.
- Реализована постобработка сущностей (нормализация значений)

**Target Text Builder**
- Удаление шумовых сущностей (подписи, служебная информация)
- Формирование `target_text` для классификации

**Классификационный модуль**
- Иерархическая схема:
  - сначала `Общая тема`
  - затем `Тематика` внутри темы
...
Решение запускается как отдельный stateless ML-сервис на FastAPI/uvicorn.  
Модели подгружаются при старте сервиса и далее используются только для инференса
Для MVP и пилота выбрана следующая архитектура:
- **Один ML-сервис**
- Запуск в:
  - Docker-контейнере
  - либо как standalone Python-сервис
- Возможность запуска:
  - CPU-only
  - или с GPU (опционально)


**Профиль 1 — CPU-only (минимально достаточный для пилота)**
- VM/контейнер: 8–16 vCPU
- RAM: 16–64 GB 
- GPU: не требуется
- Подходит для офлайн-процесса

**Профиль 2 — GPU (ускоренный)**
- 1× GPU (T4/A10 или аналог)
- VRAM: 16–24 GB
- CPU: 8–16 vCPU
- RAM: 32–64 GB
- Используется для ускорения инференса NER/CLS

#### Почему выбран этот вариант
- MVP не требует сложной системы деплоя: сервис stateless, моделей немного, логика понятная
- В пилоте источник запросов ограничен (офлайн выгрузки)
- Легко масштабируется горизонтально (реплики сервиса), если возрастёт поток обращений

**Плюсы**
- Простота деплоя
- Минимальные инфраструктурные зависимости
- Подходит для офлайн-пайплайна (`xlsx → xlsx`)
- Хорошо соответствует пилотному сценарию
**Минусы**
- Отсутствие асинхронной очереди задач
- Не оптимально для очень высоких RPS
#### Почему не более сложные альтернативы
- Очереди избыточны для MVP
- Микросервисная декомпозиция усложняет пилот

#### Масштабирование
**Горизонтальное масштабирование**
- Поднимаем несколько реплик сервиса
- Каждая реплика независима и читает модели локально

**Вертикальное масштабирование**
- Увеличение числа CPU/памяти на ноде
- Переход на GPU-инференс

### 4.3 Требования к работе системы

#### SLA и задержки

Для MVP и пилота заданы следующие ориентиры:

| Метрика | Целевое значение |
|------|------------------|
| Обработка 10 000 обращений | ≤ 1 час |
| Средняя latency на запрос | ≤ 500 мс |
...
- Ввод произвольного текста -> очень длинный `text` (десятки тысяч символов) → рост времени токенизации и инференса
...
- Сервис не хранит данные
- Все тексты обрабатываются in-memory
- Персональные данные:
  - извлекаются через NER
  - не используются как вход в классификационные модели
- Соответствует принципам минимизации обработки данных
- Нарушений GDPR не выявлено (данные не сохраняются и не передаются третьим лицам)
...
- CPU (основной источник затрат в CPU-only режиме)
- GPU (опционально, при необходимости ускорения инференса)
- Оперативная память (загрузка моделей, токенизаторов, batch-инференс)

2) **Инфраструктура**
- виртуальная машина или контейнерная среда
- хранение моделей и конфигураций
- сетевые ресурсы (в рамках внутреннего контура)

3) **Эксплуатационные издержки**
- мониторинг и логирование
- периодическое обновление и переобучение моделей
- поддержка и устранение инцидентов


### 4.7 Integration points

В рамках MVP решение реализовано в виде самостоятельного ML-сервиса с HTTP API
...
- **Health-check метод:**
  - `GET /`   используется для проверки доступности сервиса и мониторинга состояния

### 4.6 Риски

При внедрении и эксплуатации решения необходимо учитывать следующие риски и неопределенности:

- Дисбаланс и длинный хвост тематик     
  Малое число примеров для редких тематик может приводить к нестабильному качеству классификации -> агрегация редких классов, иерархическая классификация, синтетическая генерация данных.

- Обращения без целевого текста   
  Часть обращений не содержит содержательной информации, что делает автоматическую классификацию невозможной -> выявление и маркировка таких обращений, обязательная ручная обработка.

- Появление новых тематик и data drift    
  Распределение обращений может изменяться со временем -> мониторинг доли класса `Other`, периодическое дообучение моделей.

- Высокая цена ошибок маршрутизации   
  Неверная автоклассификация может привести к бизнес-ущербу -> пороги уверенности, режим shadow-mode, human-in-the-loop.

- Рост вычислительной нагрузки  
  Увеличение объема обращений или длины текста может привести к деградации производительности -> ограничение длины входного текста, batching, масштабирование сервиса


### 4.7 Нагрузочное тестирование

#### Цель
Оценить производительность и устойчивость ML-сервиса при пилотной нагрузке

#### Методика
- Эндпоинт: `POST /model`
- Режим: CPU-only
- Инструмент: `locust`
- Сценарий нагрузки: 10 одновременных пользователей (конкурентные запросы)

#### Результаты

| Параметр | Значение |
|--------|---------|
| Количество запросов | 2 000 |
| Concurrency | 10 |
| Средняя latency | ~220 мс |
| P95 latency | ~410 мс |
| Throughput | ~4.5 req/sec |
| Ошибки | 0% |
| Пиковое потребление RAM | ~6–7 GB |
